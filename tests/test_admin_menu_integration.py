...
def test_admin_menu_creates_survey(monkeypatch):
    ...
    storage_mod = importlib.reload(
        importlib.import_module("plugins_admin.storage_plugin")
    )
    storage = DummyStorage()
    monkeypatch.setattr(storage_mod, "storage", storage, raising=False)

    survey_mod = importlib.reload(
        importlib.import_module("plugins_surveys.survey_plugin")
    )
