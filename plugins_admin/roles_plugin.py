...
DEFAULT_ROLES = {
    ...
    "user": {"name": "Пользователь", "permissions": ["take_surveys"]},
    "guest": {"name": "Гость", "permissions": []},
}
...
class RolesPlugin:
    ...
    async def cmd_roles(self, message: types.Message):
        ...
        builder.button(
            text="👤 Назначить роль пользователю", callback_data="roles_assign"
        )
    ...
    async def show_user_list(self, message: types.Message):
        ...
        for user_id, name in all_users.items():
            ...
            builder.button(
                text=f"{name} - {role_name}", callback_data=f"select_user_{user_id}"
            )

        builder.button(text="⬅️ Назад", callback_data="roles_back")
        builder.adjust(1)
        markup = builder.as_markup()

        await message.edit_text(
            "Выберите пользователя для назначения роли:", reply_markup=markup
        )
    ...
    async def show_roles_list(self, message: types.Message):
        ...
        await message.edit_text(
            "Выберите роль для редактирования:", reply_markup=markup
        )
    ...
    async def handle_user_selection(
        ...
        builder.button(text="⬅️ Назад", callback_data="roles_back_to_users")
        builder.adjust(1)
        markup = builder.as_markup()

        await callback_query.message.edit_text(
            f"Выберите роль для пользователя {user_id}:", reply_markup=markup
        )
    ...
    async def process_role_name(self, message: types.Message, state: FSMContext):
        ...
        if not role_name:
            await message.reply(
                "Название роли не может быть пустым. Пожалуйста, введите название:"
            )
            return
        ...
        roles[role_id] = {"name": role_name, "permissions": []}
    ...
    async def show_permissions_editor(self, message, role_id):
        ...
        builder.button(text="💾 Сохранить", callback_data="save_permissions")
        builder.adjust(1)
        markup = builder.as_markup()
    ...
    async def handle_permission_toggle(
        self, callback_query: types.CallbackQuery, state: FSMContext
    ):
    ...
    async def handle_save_permissions(
        ...
        await callback_query.message.edit_text("✅ Разрешения роли успешно сохранены.")
