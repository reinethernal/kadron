...
class CaptchaPlugin:
    ...
    def __init__(self):
        ...
        self.primary_survey_questions = [
            {"id": "name", "text": "Как вас зовут? (Имя и фамилия)", "type": "text"},
            {
                "id": "interests",
    ...
    def is_access_granted(self, user_id: int) -> bool:
        ...
        return user_state.get("passed_primary_survey", False) or user_state.get(
    ...
    async def on_new_chat_member(self, event: ChatMemberUpdated):
        ...
        for option in options:
            builder.button(text=option, callback_data=f"captcha_{user.id}_{option}")
        builder.adjust(3)
    ...
    async def process_captcha(self, callback_query: CallbackQuery):
        ...
        if selected_option == correct_option:
            ...
            builder = InlineKeyboardBuilder()
            builder.button(
                text="Пройти короткий опрос", callback_data="start_primary_survey"
            )
            markup = builder.as_markup()
    ...
    async def show_primary_survey_question(
        self, message: Message, user_id: int, question_index: int
    ):
        if question_index >= len(self.primary_survey_questions):
            storage.set_user_state(user_id, "passed_primary_survey", True)
            await message.edit_text(
                "✅ Спасибо за заполнение опроса! Теперь у вас полный доступ."
            )
        ...
        if question["type"] == "text":
            builder = InlineKeyboardBuilder()
            builder.button(
                text="Ответить", callback_data=f"primary_text_{question['id']}"
            )
            markup = builder.as_markup()
            await message.edit_text(
            ...
            for i, option in enumerate(question["options"]):
                builder.button(
                    text=option, callback_data=f"primary_choice_{question['id']}_{i}"
                )
            builder.adjust(1)
            markup = builder.as_markup()
            await message.edit_text(
                f"{question['text']}\n\nВыберите один вариант:", reply_markup=markup
            )
            ...
            builder.button(
                text="Подтвердить", callback_data=f"primary_submit_{question['id']}"
            )
            builder.adjust(1)
            markup = builder.as_markup()
            await message.edit_text(
    ...
    async def process_primary_survey_choice(self, callback_query: CallbackQuery):
        ...
        if parts[0] == "primary" and parts[1] == "text":
            ...
            state = Dispatcher.get_current().current_state(
                user=user_id, chat=callback_query.message.chat.id
            )
            ...
            question = next(
                (q for q in self.primary_survey_questions if q["id"] == question_id),
                None,
            )
            if not question:
            ...
            if question["type"] == "single_choice":
                storage.set_user_state(
                    user_id, f"primary_answer_{question_id}", option_index
                )
                current_index = storage.get_user_state(user_id).get(
                    "primary_survey_index", 0
                )
                next_index = current_index + 1
                storage.set_user_state(user_id, "primary_survey_index", next_index)
                await self.show_primary_survey_question(
                    callback_query.message, user_id, next_index
                )
                ...
                for i, option in enumerate(question["options"]):
                    ...
                    builder.button(
                        text=text, callback_data=f"primary_choice_{question_id}_{i}"
                    )
                builder.button(
                    text="Подтвердить", callback_data=f"primary_submit_{question_id}"
                )
                builder.adjust(1)
                markup = builder.as_markup()
